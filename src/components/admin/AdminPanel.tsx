...
export function AdminPanel() {
  ...
  return (
    <div id="admin-panel-root" className="min-h-screen bg-slate-950 text-slate-100 flex flex-col md:flex-row selection:bg-amber-500 selection:text-slate-950">
            ...
            <div className="p-6 rounded-3xl bg-slate-900 border border-slate-800 shadow-xl space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400">Set multi-tier commission payout percentages across 3 levels.</p>
                </div>
              </div>
            ...
            <div className="p-6 rounded-3xl bg-slate-900 border border-slate-800 shadow-xl space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400">Automatic leadership payouts based on active paid team thresholds.</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 flex justify-between items-center text-xs">
                  <div>
                    <span className="font-bold text-amber-400 block">Tier 1: Manager</span>
                    <span className="text-[11px] text-slate-400">Min 10 Active Paid Members</span>
                  </div>
                  <span className="font-bold text-emerald-400 text-sm">৳5,000 / mo</span>
                </div>

                <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 flex justify-between items-center text-xs">
                  <div>
                    <span className="font-bold text-cyan-400 block">Tier 2: Senior Manager</span>
                    <span className="text-[11px] text-slate-400">Min 25 Active Paid Members</span>
                  </div>
                  <span className="font-bold text-emerald-400 text-sm">৳12,000 / mo</span>
                </div>

                <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 flex justify-between items-center text-xs">
                  <div>
                    <span className="font-bold text-purple-400 block">Tier 3: VIP Regional Director</span>
                    <span className="text-[11px] text-slate-400">Min 50 Active Paid Members</span>
                  </div>
                  <span className="font-bold text-emerald-400 text-sm">৳25,000 / mo</span>
                </div>
              </div>
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    showToast('error', 'Salary Distribution Failed', e.message);
                  } finally {
                ...
                className="w-full py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs shadow-md transition cursor-pointer"
              >
                {distributingSalary ? 'Distributing Salaries...' : 'Run Auto Monthly Salary Distribution Now'}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Comprehensive User Profile CRM Modal */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-xl w-full space-y-5 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <div>
                <h4 className="font-bold text-white text-base flex items-center gap-2">
                  ...
                  User 360° Profile CRM
                </h4>
                <p className="text-xs text-slate-400 font-mono">UID: {selectedUserProfileUser.id}</p>
              </div>
              <button
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <div className="p-3 rounded-2xl bg-slate-950 border border-slate-800">
                ...
                <div className="flex items-center justify-between mt-1">
                  <span className="font-mono font-bold text-white text-xs">{selectedUserProfileUser.phone}</span>
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(selectedUserProfileUser.phone);
                      showToast('info', 'Copied', 'Phone number copied to clipboard.');
                    }}
              ...
              <div className="p-3 rounded-2xl bg-slate-950 border border-slate-800">
                <span className="text-[10px] uppercase font-bold text-slate-500 block">Role & Rank</span>
                <span className="font-bold text-amber-400 text-xs block mt-1">{selectedUserProfileUser.role}</span>
              </div>

              <div className="p-3 rounded-2xl bg-slate-950 border border-slate-800">
                <span className="text-[10px] uppercase font-bold text-slate-500 block">Status</span>
                <span className={`font-bold text-xs block mt-1 ${selectedUserProfileUser.isBanned ? 'text-rose-400' : 'text-emerald-400'}`}>
                  {selectedUserProfileUser.isBanned ? 'Banned' : 'Active Member'}
                </span>
              </div>

              <div className="p-3 rounded-2xl bg-slate-950 border border-slate-800">
                <span className="text-[10px] uppercase font-bold text-slate-500 block">Account Tier</span>
                <span className="font-bold text-white text-xs block mt-1">
                  {selectedUserProfileUser.isTrial ? 'Free Trial (3 Days)' : 'VIP Paid Package'}
                </span>
              </div>

              <div className="p-3 rounded-2xl bg-slate-950 border border-slate-800">
                <span className="text-[10px] uppercase font-bold text-slate-500 block">Referral Code</span>
                <span className="font-mono font-bold text-purple-400 text-xs block mt-1">{selectedUserProfileUser.referralCode}</span>
              </div>

              <div className="p-3 rounded-2xl bg-slate-950 border border-slate-800">
                <span className="text-[10px] uppercase font-bold text-slate-500 block">Free Withdraw</span>
                <span className={`font-bold text-xs block mt-1 ${selectedUserProfileUser.freeWithdrawAllowed ? 'text-emerald-400' : 'text-amber-400'}`}>
                  {selectedUserProfileUser.freeWithdrawAllowed ? 'Permitted' : 'Blocked'}
                </span>
              </div>
            </div>
            ...
            <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
              <span className="text-[10px] uppercase font-bold text-slate-500 block">Hardware / Device Fingerprint</span>
              <div className="flex items-center justify-between">
                <span className="font-mono text-cyan-400 text-xs">{selectedUserProfileUser.deviceFingerprint || 'Unknown Device'}</span>
                {selectedUserProfileUser.deviceFingerprint && (
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(selectedUserProfileUser.deviceFingerprint);
                      showToast('info', 'Copied', 'Device fingerprint copied.');
                    }}
            ...
            <div className="space-y-2 pt-2 border-t border-slate-800">
              <span className="text-xs font-bold text-slate-400 uppercase block">Admin Action Center</span>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                <button
                  onClick={() => {
                    const newPw = prompt('Enter new password for this user (minimum 6 characters):');
                    if (newPw && newPw.length >= 6) {
                      apiRequest(`/api/admin/users/${selectedUserProfileUser.id}/reset-password`, {
                        method: 'POST',
                        body: JSON.stringify({ newPassword: newPw }),
                      })
                        .then(() => showToast('success', 'Password Reset', 'New user password set successfully.'))
                        .catch((err) => showToast('error', 'Reset Failed', err.message));
                    }
                  }}
                  className="p-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold flex items-center justify-center gap-1.5 transition"
                >
                ...
                <button
                  onClick={() => {
                    handleToggleBan(selectedUserProfileUser);
                    setSelectedUserProfileUser({
                      ...selectedUserProfileUser,
                      isBanned: !selectedUserProfileUser.isBanned,
                    });
                  }}
                  className={`p-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition ${
                    selectedUserProfileUser.isBanned
                      ? 'bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500 hover:text-slate-950'
                      : 'bg-rose-500/20 text-rose-400 hover:bg-rose-500 hover:text-white'
                  }`}
                >
                  <Power className="w-3.5 h-3.5" />
                  {selectedUserProfileUser.isBanned ? 'Unban Account' : 'Ban Account'}
                </button>

                <button
                  onClick={() => {
                    handleToggleFreeWithdraw(selectedUserProfileUser);
                    setSelectedUserProfileUser({
                      ...selectedUserProfileUser,
                      freeWithdrawAllowed: !selectedUserProfileUser.freeWithdrawAllowed,
                    });
                  }}
                  className="p-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-amber-300 text-xs font-bold flex items-center justify-center gap-1.5 transition"
                >
                ...
                <button
                  onClick={() => {
                    apiRequest(`/api/admin/users/${selectedUserProfileUser.id}/reset-trial`, { method: 'POST' })
                      .then(() => {
                        showToast('success', 'Trial Reset', '3-day free trial reset for user.');
                        loadAllAdminData();
                      })
                      .catch((err) => showToast('error', 'Error', err.message));
                  }}
                  className="p-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-cyan-300 text-xs font-bold flex items-center justify-center gap-1.5 transition"
                >
                  <RefreshCw className="w-3.5 h-3.5" />
                  Reset 3-Day Trial
                </button>
                ...
                <button
                  onClick={() => {
                    setBalanceModalUser(selectedUserProfileUser);
                  }}
                  className="p-2.5 rounded-xl bg-emerald-500/20 hover:bg-emerald-500 text-emerald-300 hover:text-slate-950 text-xs font-bold flex items-center justify-center gap-1.5 transition"
                >
                  <DollarSign className="w-3.5 h-3.5" />
                  Adjust Balance
                </button>
                ...
                <button
                  onClick={() => {
                    const newRole = prompt('Assign Badge Role (Member, Manager, Senior Manager, Regional Director, VIP):', selectedUserProfileUser.role);
                    if (newRole) {
                      apiRequest(`/api/admin/users/${selectedUserProfileUser.id}/role`, {
                        method: 'POST',
                        body: JSON.stringify({ role: newRole }),
                      })
                        .then(() => {
                          showToast('success', 'Role Updated', `User promoted to ${newRole}`);
                          loadAllAdminData();
                          setSelectedUserProfileUser({ ...selectedUserProfileUser, role: newRole as any });
                        })
                        .catch((err) => showToast('error', 'Role Update Failed', err.message));
                    }
                  }}
                  className="p-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-purple-300 text-xs font-bold flex items-center justify-center gap-1.5 transition"
                >
